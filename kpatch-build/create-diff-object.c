...
static bool insn_is_load_immediate(struct kpatch_elf *kelf, void *addr)
{
	...
	case LOONGARCH:
		/* arg2: addi.w r5, r5, imm */
		if ((*(unsigned int *)insn & 0xffc003ff) == 0x28000a5)
			return true;
		/* arg2: ori r5, r5, imm */
		if ((*(unsigned int *)insn & 0xffc003ff) == 0x38000a5)
			return true;
		/* arg3: addi.w r6, r6, imm */
		if ((*(unsigned int *)insn & 0xffc003ff) == 0x28000c6)
			return true;
		/* arg3: ori r6, r6, imm */
		if ((*(unsigned int *)insn & 0xffc003ff) == 0x38000c6)
			return true;
		break;
...
static struct special_section special_sections[] = {
	{
		.name		= "__bug_table",
		.arch		= X86_64 | PPC64 | S390 | LOONGARCH,
...
static void kpatch_check_relocations(struct kpatch_elf *kelf)
{
	...
	/* LoongArch rela's SYM+addend maybe outside of range, but it is ok */
	if (kelf->arch == LOONGARCH) {
		log_debug("do not check relocations for LoongArch\n");
		return;
	}
...
#ifdef __loongarch__
		(rela->type == R_LARCH_MARK_LA ||
		rela->type == R_LARCH_SOP_PUSH_ABSOLUTE ||
		rela->type == R_LARCH_SOP_PUSH_PLT_PCREL)
#else
						...
						char *pmod_name)
{
			...
			if (kelf->arch == LOONGARCH) {
				...
				list_for_each_entry_continue(rela, &relasec->relas, list) {
					...
					if (list_prev_entry(rela, list)->need_klp_reloc) {
						nr++;
						rela->need_klp_reloc = 1;
					} else {
						rela->need_klp_reloc = 0;
					}
				}
				rela = list_prev_entry(rela, list);
			}
		}
	}
	...
	list_for_each_entry(relasec, &kelf->sections, list) {
		...
		list_for_each_entry_safe(rela, safe, &relasec->relas, list) {
			...
			if (kelf->arch == LOONGARCH && !strlen(rela->sym->name)) {
				/* LoongArch rela's symbol may be NULL, don't lookup it */
			} else if (!lookup_symbol(table, rela->sym, &symbol))
...
static void kpatch_create_mcount_sections(struct kpatch_elf *kelf)
{
	...
	list_for_each_entry(sym, &kelf->symbols, list) {
		...
		case LOONGARCH: {
			...
			if (*insn == 0x03400000 && *(insn + 1) == 0x03400000)
				found = true;
...
static void kpatch_find_func_profiling_calls(struct kpatch_elf *kelf)
{
	...
	list_for_each_entry(sym, &kelf->symbols, list) {
		...
		case S390:
			...
			break;
		case LOONGARCH:
			patch_sec = find_section_by_name(&kelf->sections,
							"__patchable_function_entries");
			if (patch_sec) {
				list_for_each_entry(rela, &patch_sec->rela->relas, list) {
					if (rela->sym->sec == sym->sec &&
						(rela->sym->sym.st_value + rela->addend == sym->sym.st_value)) {
						sym->has_func_profiling = 1;
						break;
					}
				}
